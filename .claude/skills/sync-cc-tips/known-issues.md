...
| 2026-09-06 | 第五步版本升级段与 AGENTS.md 版本管理规则直接冲突：要求升 `.claude-plugin/marketplace.json` 的**顶层** `version`（该字段按规范仅在增删插件时升，且 Patch 位永久停在 0），完全未提 `plugins/*/.claude-plugin/plugin.json`（真正该升的两份之一），并把 `.codex-plugin/plugin.json` 描述成「从 marketplace.json 抄录，不是独立真源」（AGENTS.md 明确两份无主从关系）。照此执行会升错文件、漏升一份，且触发 `commit-cc-plugin` 的同值校验阻断 | `/sync-cc-tips`（v2.1.259→v2.1.263 同步），执行时发现冲突后按 AGENTS.md 处理 | 已修复 | 1.4.0 |
